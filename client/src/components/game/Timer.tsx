...
interface TimerProps {
  ...
  timeLimit: number;
  /** Server timestamp when question started (ms) */
  startTime: number;
 ...
 */
export function Timer({ timeLimit, startTime, onExpire, size = 80 }: TimerProps) {
  ...
  return (
    <svg
      ...
      height={size}
      role="timer"
      aria-label="Question countdown timer"
      className="shrink-0"
    >
      {/* Background ring */}
      <circle
      ...
      <text
        ...
        textAnchor="middle"
        className="font-display font-bold fill-current text-gray-800"
        fontSize={size * 0.32}
      >
        {timeLimit}
      </text>
