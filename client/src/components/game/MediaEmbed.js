...
export function MediaEmbed({ url, type }) {
    ...
    if (kind === "youtube") {
        ...
        return (_jsx("div", { className: `w-full ${heightClass} rounded-xl overflow-hidden bg-black shadow-lg mb-2 relative pointer-events-none`, children: _jsx("iframe", { src: embed, allow: "autoplay; encrypted-media", title: type === "audioclip" ? "Music clip" : "Video clip", className: "absolute inset-0 w-[110%] h-[120%] -left-[5%] -top-[10%] border-0 pointer-events-none" }) }));
    }
    if (kind === "audio") {
